...
class MenuItemDTFunction : public MenuItemInterface
{
    private:
        // Function we call when element is clicked (ptr to it)
        std::function<int(DynamicArray<Type>*)> func;
        DynamicArray<Type> *dt;

    public:

        MenuItemDTFunction() : MenuItemInterface(), func(nullptr), dt(nullptr){}
        MenuItemDTFunction(std::string label, std::function<int(DynamicArray<Type>*)> func, DynamicArray<Type> *dt) : MenuItemInterface(label), func(func), dt(dt){}
        int clicked()
        {
...
/**
 * Class that represents menu button that should run given function and pass pointer to Graph as argument. It stores label and function that should be called when element is selected and ENTER key is pressed.
 * 
 */
template <typename Type>
class MenuItemGraphFunction : public MenuItemInterface
{
    private:
        // Function we call when element is clicked (ptr to it)
...
class MenuDt : public Menu
{
    ...
    public:
        ...
        MenuDt(DynamicArray<Type> *ptr_to_dt) : Menu(), dt(ptr_to_dt) {}
        ~MenuDt()
        {
            delete dt;
            dt = nullptr;
        }
        void add_item_dt(std::string label, std::function<int(DynamicArray<Type>*)> func)
        {
        ...
        int display_menu()
        {
            ...
            while (true)
            {
                ...
                mvwprintw(menu_window, menu_box_h-4, COLS/2-13, "Projekt analiza algorytmów");
                mvwprintw(menu_window, menu_box_h-3, COLS/2-11, "by Jędrzej Boruczkowski");
...
/**
 * Class representing menu that interacts with graph. It stores data structure of given type and can have menu items that call functions that edit given graph
 * 
 **/
template <typename Type>
class MenuGraph : public Menu
{
    ...
    public:
        ...
        void add_item_dt(std::string label, std::function<int(GraphInterface<Type>*)> func)
        {
            if (items_number<MAX_ITEMS)
            {
                items[items_number] = new MenuItemGraphFunction<Type>(label, func, dt);
                items_number++;
            }
            else
            {
